...
// typed alias for destinations including durations
type DestinationWithDurations = Prisma.DestinationGetPayload<{
  include: { durations: { select: { id: true; title: true; priceFrom: true; currency: true; days: true; priceModel: true } } };
}>;
...
async findOne(
  ...
  if (!destination) {
    throw new NotFoundException(`Destination not found: ${slug}`);
  }

  // ✅ Use first duration as base
  const firstDur = destination.durations?.[0];
  const baseCurrency = (firstDur?.currency || 'USD').toUpperCase();
  ...
  const convertedPrice =
    origPrice != null ? Math.round(origPrice * rate * 100) / 100 : null;
  ...
  async findAll(
    ...
    // decide up-front if we need durations in the DB payload
    const needDurations = doConvert || sortBy === 'price-asc' || sortBy === 'price-desc';

    // fetch raw data with or without durations
    let raw: any[] = [];
    if (featured === 'true') {
    ...
    // Now safe to inspect d.durations because `raw` will include them when needDurations === true
...
return {
  ...
  durations: convertedDurations ?? d.durations,
    ...
    // sort by converted price if requested
 ...
 @Post()
  async create(@Body() body: any, @Res() res: Response) {
    try {
      const created = await this.service.createDestination(body);
      return res.status(201).json({ data: created });
    } catch (err) {
      this.logger.error('create destination failed', err);
  ...
  async update(@Param('slug') slug: string, @Body() body: any, @Res() res: Response) {
    try {
      const updated = await this.service.updateDestinationBySlug(slug, body);
      return res.status(200).json({ data: updated });
    } catch (err) {
      this.logger.error('update destination failed', err);
...
async remove(@Param('slug') slug: string, @Res() res: Response) {
  try {
await this.service.deleteBySlug(slug);
    // 204 No Content is appropriate for successful delete
return res.status(200).json({ deleted: true, identifier: slug });
  } catch (err: any) {
...
/**
 * POST /destinations/bulk-delete
 * body: { ids: string[], by?: 'id' | 'slug' }  // by defaults to 'id'
 */
@Post('bulk-delete')
async bulkDelete(
  @Body() body: { ids?: string[]; by?: 'id' | 'slug' },
  @Res() res: Response,
) {
  try {
    ...
    // Prisma.deleteMany returns BatchPayload { count: number }
    const deletedCount = (result && typeof (result as any).count === 'number') ? (result as any).count : 0;
...
async removeGalleryItem(
  ...
  try {
    // find destination id
    const existing = await this.service.findBySlugOrId(slug); // we'll add this helper in service
    if (!existing) {
      return res.status(404).json({ error: `Destination not found: ${slug}` });
    }
    const destinationId = existing.id;

    const { id, imageUrl } = body ?? {};
    if (!id && !imageUrl) {
      return res.status(400).json({ error: 'Provide id or imageUrl to delete' });
    }

    let result;
    if (id) {
      result = await this.service.deleteGalleryById(destinationId, id);
    } else {
      result = await this.service.deleteGalleryByUrl(destinationId, imageUrl!);
    }
...
/**
 * GET /destinations/:slug/nearby?radius=5&limit=10
 */
@Get(':slug/nearby')
async findNearby(
  @Param('slug') slug: string,
  @Req() req: Request,
  @Res() res: Response,
  @Query('radius') qRadius?: string,
  @Query('limit') qLimit?: string,
) {
  try {
    const radius = qRadius ? Number(qRadius) : 5;
    const limit = qLimit ? Math.min(Math.max(1, Number(qLimit)), 50) : 10;
    ...
    if (Number.isNaN(radius) || radius <= 0 || !isFinite(radius)) {
      return res.status(400).json({ error: 'Invalid radius' });
    }

    // call service
    this.logger.debug(`findNearby: slug=${slug} radius=${radius} limit=${limit}`);
    const items = await this.service.findNearby(slug, radius, { limit });

    // cache hint (short)
    res.setHeader('Cache-Control', 'public, s-maxage=60, stale-while-revalidate=300');
    res.setHeader('Vary', 'Accept-Encoding, Cookie, X-Country');

    return res.status(200).json({ data: items, meta: { count: items.length, radiusKm: radius, limit } });
  } catch (err: any) {
    this.logger.error('findNearby failed', err);
    if (err instanceof NotFoundException) {
      return res.status(404).json({ error: err.message });
    }
    return res.status(500).json({ error: err?.message ?? 'Server error' });
